...
class InternLMXComposer2ForCausalLM(InternLM2PreTrainedModel):
    ...
    def encode_img(self, image, hd_num=25):
        ...
        if isinstance(image, str):
            ...
            if ext.lower() in image_extensions:
                ...
                image = Image_transform(image, hd_num = hd_num) # it will resize and pad to size [g_h, g_w] : [k1 x B, k2 x B], with k1xk2 <= hd_num
                #HW=1100-825 #new HW=1680-1680 #HD=9 #image.size=(1680,1680)
                ...
                return None
            image = self.vis_processor(image).unsqueeze(0).to(self.device) # normalizes the image (preprocessing)
            
        else:
            assert isinstance(image, torch.Tensor)

        img_embeds, atts_img, img_target = self.img2emb(image)
        #img_embeds: [1,4081,4096] : dim'=4096 as in my notes in the paper highlighted by red
        return img_embeds # this is the output of P_v

    def img2emb(self, image):
        #image: 1680x1680
        img_embeds, img_split = self.vit([image],  # this ViT is our 'internlm/internlm-xcomposer2d5-clip' encoder I(.)
            self.plora_glb_GN, self.plora_sub_GN) #GNs are our seperators. <s_g> in the paper
        #img_embeds:4081x4096 {on the last white page of the paper I showed how this is achieved} => they reshape ViT outputs: (N, C) => (N/4, 4C)
        if len(img_split) > 1:
            print ('Batch Size >1 is not supported.')
            assert 0
        img_embeds = self.vision_proj(img_embeds) # this is our P_v: mlp2x_gelu
        #img_embeds:4081x4096
        atts_img = torch.ones(
        ...
        return img_embeds, atts_img, img_target#img_embeds:4081x4096
    ...
    def interleav_wrap_chat(self, query, image, history = [], meta_instruction='', max_length=16384, hd_num=24):
        ...
        for record in history:
            ...
            #==========> history enable us to use previous conversations and have a multiple-run conversation.

        prompt += f"""[UNUSED_TOKEN_146]user\n{query}[UNUSED_TOKEN_145]\n[UNUSED_TOKEN_146]assistant\n"""
        #==========> Adding the actual query made by the user

        image_nums = len(image)
        if image_nums == 1 and prompt.find('<ImageHere>') == -1:
            #print ('auto append image at the begining')
            prompt = '<ImageHere>' + prompt
            # Have this placeholder prepared. Because the user might not wanna attach an image. If so, we dont need <ImageHere>

        parts = prompt.split('<ImageHere>') # if the user has fed an image: ['', '$prompt'], otherwise: [$prompt]
        ...
        if image_nums > 1: # so the model can also handle temporal images MAYBE -------> However {chat.py {LINE72} does not allow this. We should change the code for multi-image input}
            hd_num = 6     # later I understood that the user can also give a path to a video file. So problem solved!! We can handle multi-image inputs   
        else:
            hu_num = hd_num # we mostly enter here
        for idx, part in enumerate(parts): #----------> we perform this either once or twice depending on whether user has entered image_path
        # THIS {IF} HANDLE TEXT TOKENIZATION AND EMBEDDING and is ran only once and embedd all words of the input text
            if need_bos or len(part) > 0: # len(part) > 0 will drop the first element of ['', '$prompt'] and only get the `part <- $prompt`
                ...
                if need_bos:
                    need_bos = False # this will avoid the for loop to again consider a begining-of-sentence token

                part_embeds = self.model.tok_embeddings(
                    part_tokens.input_ids)
                wrap_embeds.append(part_embeds) # we are preparing out text token embeddings to be fed to the transformer. [sq_len=25, 4096] (25: as an example)
                wrap_im_mask.append(torch.zeros(part_embeds.shape[:2]))
                temp_len += part_embeds.shape[1] # update the length (/16384)
        # THIS {IF} HANDLE IMAGE TOKENIZATION AND EMBEDDING and is ran only once
            if idx < image_nums:
                img = self.encode_img(image[idx], hd_num) # image tokens after P_v() ready for the LLM => [4081, 4096]
                wrap_embeds.append(img) # will be ran only once 
                wrap_im_mask.append(torch.ones(img.shape[:2]))
                temp_len += img.shape[1] # update the length (/16384)
            #temp_len: 4081(IMGE)+1(SEP)+25(TEXT)
            if temp_len > self.max_length: # Obvious
                break
    
        wrap_embeds = torch.cat(wrap_embeds, dim=1) # add IMAGE_PART to the TEXT_PART
        #wrap_embeds: [4107,4096]
        wrap_im_mask = torch.cat(wrap_im_mask, dim=1)
        wrap_embeds = wrap_embeds[:, :self.max_length].to(self.device) # should not change anything basically!
        wrap_im_mask = wrap_im_mask[:, :self.max_length].to(self.device).bool()
        inputs = { # input ready for LLM (IMAGE + TEXT)
            'inputs_embeds': wrap_embeds #wrap_embeds: [4107,4096]
        }
    ...
    def forward(self,
        ...
        logits = self.output(hidden_states) #projection from d_model to target_vocab_size
        logits = logits.float()
