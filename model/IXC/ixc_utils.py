...
def Image_transform(img, hd_num=25):
    ...
    trans = False
    if width < height: # if the image is Portrait, we make it Landscape. The body will rotate the image 90 degree
        img = img.transpose(Image.TRANSPOSE)
        trans = True
        width, height = img.size
    # height > width ALWAYS
    ratio = (width/ height) # ratio < 1.0 ALWAYS
    scale = 1
    while scale*np.ceil(scale/ratio) <= hd_num:
        scale += 1
    scale -= 1
    scale = max(np.floor(width / 560), scale) # this is K2 in the paper
    new_w = int(scale * 560)    # g_w = K2 x B
    new_h = int(new_w / ratio)  # g_h = K1 x B     ===> K1 x K2 < hd_num (:=P)

    img = transforms.functional.resize(img, [new_h, new_w],) # now we are [g_h, g_w]
    #g_w is alread dividable by 560
    #g_h might not be, because we are doing new_h = int(new_w / ratio). To make it dividable by B, we run this
    img = padding_336(img, 560)
    width, height = img.size
    print(f'new HW : {width} {height}')
    print(f'number of patches : { int((width/560 * height/560) )}')
    if trans:
        img = img.transpose(Image.TRANSPOSE) # we make the image again Portrait with width < height
